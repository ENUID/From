...
export default function AdminCommunityPage() {
  ...
  // Loads the member list separately. Errors here are shown inline but
  // never log you out — you can still add/remove members.
  const loadList = useCallback(async (s: string) => {
    ...
    try {
      ...
      if (!r.ok) {
        setListErr(d.detail ?? d.error ?? `Couldn't load list (${r.status})`)
        return
      }
      setList(d.list ?? [])
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    checkAuth(stored).then(ok => {
      if (ok) { setView('admin'); loadList(stored) }
  ...
  if (view === 'loading') {
    return <div style={{ ...st.page, ...st.center }}><div style={{ color: '#555', fontSize: '14px' }}>Loading…</div></div>
  }
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
          ...
          <button onClick={() => { sessionStorage.removeItem(STORAGE_KEY); setSecret(''); setView('login') }}
            style={{ background: 'none', border: 'none', color: '#555', fontSize: '13px', cursor: 'pointer' }}>Sign out</button>
        </div>
        ...
        <div style={{ background: '#1a1a1a', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
          ...
          <input type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)}
            style={{ ...st.input, marginBottom: '10px' }} />
          ...
          <button onClick={grant} disabled={working || !email.trim()} style={st.btn(!working && !!email.trim())}>
            {working ? '…' : 'Grant access'}
          </button>
        </div>

        {listErr && (
          <div style={{ background: '#2a1212', border: '1px solid #5a2020', borderRadius: '12px', padding: '14px 16px', marginBottom: '20px' }}>
            <div style={{ color: '#f88', fontSize: '13px', marginBottom: '8px' }}>{listErr}</div>
            <button onClick={() => loadList(sessionStorage.getItem(STORAGE_KEY) ?? '')}
              style={{ padding: '6px 12px', borderRadius: '7px', border: '1px solid #5a2020', background: 'transparent', color: '#f88', fontSize: '13px', cursor: 'pointer' }}>
              Retry
            </button>
          </div>
        )}
        ...
        {list.length > 0 ? (
          ...
          !listErr && <div style={{ color: '#444', fontSize: '14px', textAlign: 'center', paddingTop: '12px' }}>No members yet</div>
        )}
      </div>
    </div>
  )
