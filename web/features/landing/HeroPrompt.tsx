...
/**
 * Hero search bar for the landing page — a faux prompt that cycles through real
 * example queries, then drops the visitor into the app. Daydream's "describe it
 * and we find it" promise, in FROM's voice.
 */
...
export default function HeroPrompt() {
  ...
  // Typewriter cycle through the example prompts.
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (typing) {
        ...
        hold++
        if (hold > 22) {            // pause on the full line
